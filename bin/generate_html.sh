...
diff output_html/playbooks.csv* | grep ';' | grep -v '^. ;' | sed 's/^..//'  | awk -F ';' '{print $1}'  | sort -u  | while read playbook ; do ls --full-time output_html/playbooks/$playbook".html" | awk '{print $6" "$7" '$playbook'"}' ; done  | sort -r  | awk 'BEGIN {updated=""; print "<?xml version=\"1.0\" encoding=\"utf-8\"?><feed xmlns=\"http://www.w3.org/2005/Atom\"><title>Ansible checks</title>"} {if ( updated == "" ) updated=$1" "$2 ; print "<entry><title>ansible check "$3" changed</title><id>"$3$1$2"</id><link>'$ANSIBLECHECK_URL'/playbook/"$3".html</link><updated>"$1" "$2"</updated></entry>"} END{print "<updated>"updated"</updated></feed>"}' > output_html/feed.xml
