...
for config in config/*inc ; do
	...
	for playbook in $PLAYBOOKS_TO_RUN ; do
		...
		$ANSIBLE_PLAYBOOK_CMD --ssh-extra-args "-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no" --diff --check $playbook 2>&1 | aha -t "ansible playbook $playbook" > $actualdir/output_html/playbooks/$html
	done
	cd - 2> /dev/null
